...
public class TokenUtils {
    ...
    public static String encodeJwt(String payload, Date date) throws Exception {
        return Jwts.builder()
                .signWith(Keys.hmacShaKeyFor(readPrivateKey().getBytes()), SignatureAlgorithm.HS512)
    ...
    public static String decodeJwt(String token) throws Exception {
        Jws<io.jsonwebtoken.Claims> parsedToken = Jwts.parserBuilder()
                .setSigningKey(readPrivateKey().getBytes())
                .requireAudience("Sign In")
                .requireIssuer("Customer")
                .build()
    ...
    public static String readPrivateKey() throws Exception {
        InputStream contentIS = TokenUtils.class.getResourceAsStream("/privateKey.pem");
        byte[] tmp = new byte[2048];
        int length = contentIS.read(tmp);
        String key = new String(tmp, 0, length, "UTF-8");
        return removeBeginEnd(key);
    }
